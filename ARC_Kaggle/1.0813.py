...
filterwarnings(action='ignore', category=DeprecationWarning, message='`np.bool`')
...
visu_path = data_path / 'visual'

visu_path.mkdir(exist_ok=True)
...
def visualize(task, task_id):
    ...
    plt.tight_layout()
    plt.savefig((visu_path / (task_id + ".jpg" )).resolve())
    plt.clf()
    
def plot_pic(task, name):
    plt.imshow(np.array(task), cmap=cmap, norm=norm)
    plt.savefig((visu_path / (name + ".jpg" )).resolve())
    plt.clf()
...
for task_id, task in tqdm(data.items()):
    ...
    for pair_id in range(originalT.nTest):
        print("len(finalPredictions[pair_id])", len(finalPredictions[pair_id]))
        
        pred = []
        for i in range(len(finalPredictions[pair_id])):
            pred.append(flattener(finalPredictions[pair_id][i].astype(int).tolist()))
            
            fig_name = f"{task_id}_test{pair_id}_pred{i}_score{bestScores[i]}"
            plot_pic(finalPredictions[pair_id][i], name=fig_name)
        predictions = pred
...
print("#" * 60)
print("temp submission: ", submission)
# In[143]:
